...
{
  return (hebiGroupSendFeedbackRequest(internal_) == 0);
}

bool Group::getNextFeedback(GroupFeedback* feedback, int timeout_ms)
{
  return (hebiGroupGetNextFeedback(internal_, feedback->internal_, timeout_ms) == 0);
}
...
{
  return (hebiGroupStartLog(internal_, dir.c_str(), nullptr) == 0);
}

bool Group::startLog(std::string dir, std::string file)
{
  return (hebiGroupStartLog(internal_, dir.c_str(), file.c_str()) == 0);
}
